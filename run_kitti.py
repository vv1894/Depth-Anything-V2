...
if __name__ == '__main__':
    ...
    def get_image_path(data_path, folder, frame_index, side):
    ...
    if os.path.isfile(args.img_path):
        if args.img_path.endswith('txt'):
            with open(args.img_path, 'r') as f:
                file_rel_pos = f.read().splitlines()
            for path in file_rel_pos:
                line = path.split()
                folder = line[0]
                frame_index = int(line[1])
                side = line[2]
                for i in range(-1, 2):
                    full = get_image_path(args.img_dir, folder, frame_index+i, side)
    ...
    for k, filename in enumerate(filenames):
        ...
        target_path = args.outdir + filename[len(args.img_dir):len(filename)-3] + 'png'
        # check existed files
        ...
        if args.pred_only:
            path = args.outdir + filename[len(args.img_dir):len(filename)-3] + 'png'
